...
ENT.Type 			= "anim"
ENT.Base 			= "base_anim"
ENT.PrintName		= "Fast Zombie Spawner"
...
if SERVER then
	...
	function ENT:Think()
		...
		if !self.ent1:IsValid() then
			if self.torsoChance == 1	and	IsMounted( "ep2" ) then	self.ent1 = ents.Create("npc_fastzombie_torso") else	self.ent1 = ents.Create("npc_fastzombie") end
			self.ent1:SetPos(self:GetPos())
			if ConVarExists( "npcg_randomyaw" ) and GetConVarNumber( "npcg_randomyaw" ) == 0 then
			self.ent1:SetAngles( Angle( 0 , 0 , 0 ) )
		else
		...
		if !self.ent2:IsValid() then
			...
			self.ent2:SetHealth( GetConVarNumber("npcg_healthoverride_zmb") )
			self.ent2:SetKeyValue( "spawnflags", tostring( self.kvNum + self.longNum + self.pushNum + self.fadeNum ) )
			self.ent2:SetKeyValue( "wakeradius", GetConVarNumber( "npcg_wakeradius_zmb" ) )
			self.ent2:Spawn()
			self.ent2:Activate()
		...
		if !self.ent3:IsValid() then
			if self.torsoChance == 1	and	IsMounted( "ep2" ) then	self.ent3 = ents.Create("npc_fastzombie_torso") else	self.ent3 = ents.Create("npc_fastzombie") end
			...
			self.ent3:SetHealth( GetConVarNumber("npcg_healthoverride_zmb") )
			self.ent3:SetKeyValue( "spawnflags", tostring( self.kvNum + self.longNum + self.pushNum + self.fadeNum ) )
			self.ent3:SetKeyValue( "wakeradius", GetConVarNumber( "npcg_wakeradius_zmb" ) )
			self.ent3:Spawn()
			self.ent3:Activate()
